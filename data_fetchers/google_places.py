...
class GooglePlaces(BaseFetcher):
    ...
    def parse_data(self, raw_data):
        ...
        if not results:
            self.logger.warning("Brak wynikow w odpowiedzi API.")
            return []
        ...
        for i, result in enumerate(results, start=1):
            ...
            if details:
                ...
                self.logger.warning(f"[{i}/{total}] Brak szczegolow dla place_id={place_id}")

            # delay zeby nie dostac bana
            time.sleep(0.5)

        self.logger.info(f"Parsowanie zakonczone. Pobrano {len(data)} poprawnych rekordow.")
        return data
